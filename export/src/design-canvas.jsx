...
function DCViewport({ children, minScale = 0.1, maxScale = 8, style = {} }) {
  ...
  React.useEffect(() => {
    ...
    const onWheel = (e) => {
      // If the wheel is over an artboard with an internal scrollable region
      // (and it's not a pinch-zoom gesture), let the browser scroll the
      // artboard contents natively instead of panning/zooming the canvas.
      if (!(e.ctrlKey || e.metaKey)) {
        const inSlot = e.target.closest && e.target.closest('[data-dc-slot]');
        if (inSlot) {
          let n = e.target;
          while (n && n !== inSlot) {
