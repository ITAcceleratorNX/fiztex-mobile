...
**С телефона генерируется только конспект.** Тест — нет, и это не недоделка: его
результат это вопросы с ключом, а редактора вопросов в приложении нет, и сгенерировать
то, чего нельзя прочитать и поправить, значит отдать классу непроверенный машинный тест.
Конспект ложится в `description`, который правит обычная форма задания.
