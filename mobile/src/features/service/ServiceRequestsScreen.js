...
import { useServiceRequestList } from '@shared/hooks/useServiceRequests';
...
export function ServiceRequestsScreen({ nav, root = false }) {
  ...
  const { loading, error, rows, reload, refresh, refreshing } = useServiceRequestList(section);
...
function Body({ loading, error, rows, section, headers, onRetry, onOpen }) {
