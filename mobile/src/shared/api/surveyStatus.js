...
/**
 * Тест школьного психолога (PSYCHOLOGIST-002) — это опрос с `origin=PSYCHOLOGICAL`, и приходит
 * он той же лентой `GET /api/surveys/my`, что и школьные опросы. Разводит их одно это поле:
 * тест живёт в плитке «Тест от школьного психолога», а не под баннером опроса школы, и после
 * отправки говорит своими словами. Правило здесь, а не в экранах: главная, раздел «Опросы» и
 * экран прохождения обязаны одинаково решать, что считать тестом.
 */
export function isPsychTest(survey) {
  return survey?.origin === 'PSYCHOLOGICAL';
}
